...
var tblClothing = GetDomElement("tblSORerefence");
...
GetDomElement("btnGenerateReport").addEventListener('click', function () {
    ...
    if (bValidForm) {
        ...
        } else if (soList.code == 204 || soList.code == 404) {
        } else {
...
function TasAsReceivedByLogistics(soReference) {
    ...
    GetDomElement("btnConfirmTwo").onclick = ()=>{
        var doneItem = InvokeService(GetURL(), `LogisticNonIndustrialMethods/receivedcompleteditembylogistics/${soReference}`, "POST", "");
...
function DeliverItem(soReference) {
    ...
    GetDomElement("btnConfirmTwo").onclick = ()=>{
        var doneItem = InvokeService(GetURL(), `LogisticNonIndustrialMethods/deliveritem/${soReference}`, "POST", "");
        if (doneItem.code == 200) {
            var oData = JSON.parse(doneItem.data);
            if (oData.code == 200) {
...
//     var map = GetDomElement("map");
//     var apikey = "AIzaSyAsWgdZ0oBdiSfRlw4isBaZFefCn7MEWmk";
//     var pinLocation = "ChIJrTLr-GyuEmsRBfy61i59si0"
//     map.src = `https://www.google.com/maps/embed/v1/place?key=${apikey}&q=${pinLocation}`;
