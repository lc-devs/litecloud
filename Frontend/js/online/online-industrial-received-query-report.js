GetDomElement('btnSearchReport').addEventListener('click', function () {
    ...
    if (BValidForm) {
        ...
        var oData= InvokeService(GetURL(), `LogisticIndustrialMethods/receivedqueryreport?dateFrom=${dateFrom}&dateTo=${dateTo}&customerID=${customerId}'}`, 'GET', '');
        if (oData.code == 200) {
...
function populateTableQuery(parsedData) {
    ...
    for (var i = 0; i < parsedData.length; i++) {
        ...
        divtbl.innerHTML +=
            "<tr> " +
            "<td class='border text-center px-0 text-success so-reference' data-toggle='modal' onclick='generateSODetails()''> " +`${parsedData[i].so_reference == 0 ? "<label class='text-danger'>For pickup</label>": parsedData[i].so_reference }`+ "</td>" +
...
window.addEventListener('load', function () {
    ...
    var oData= InvokeService(GetURL(), `LogisticIndustrialMethods/receivedqueryreport?dateFrom=${dateFrom}&dateTo=${dateTo}&customerID=0'}`, 'GET', '');
...
GetDomElement('pdfile').addEventListener('click', function () {
    ...
    Download(`PDFGenerator/logisticreceivedreport?dateFrom=${dateFrom}&dateTo=${dateTo}&customerID=${customerId}&isIndustrial=true`, `received-report${dateFrom}_${dateTo}`);
