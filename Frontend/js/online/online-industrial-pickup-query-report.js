GetDomElement('btnSearchReport').addEventListener('click', function () {
    ...
    if (BValidForm) {
        ...
        var oData= InvokeService(GetURL(), `PickupIndustrialMethods?dateFrom=${dateFrom}&dateTo=${dateTo}&customerID=${customerId}&pickupBy=${pickUpby}&isAllEntries=${getAllEntries()}&isForPickupOnly=${getAllPickUp()}`, 'GET', '');
        if (oData.code == 200) {
...
function populateTableQuery(parsedData) {
    ...
    for (var i = 0; i < parsedData.length; i++) {
        divtbl.innerHTML +=
            "<tr> " +
            "<td class='border text-center px-0 text-success so-reference' data-toggle='modal' > " +`${parsedData[i].so_reference == 0 ? "<label class='text-danger'>For pickup</label>": parsedData[i].so_reference }`+ "</td>" +
