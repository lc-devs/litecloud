...
GetDomElement("btnExcelFile").addEventListener('click', function () {
    var pdfTitle = "Laundry Non Industrial Query Report";
    ...
    fnExcelReport(pdfTitle, "SaveTable", `${dateGenerated}-non_industrial_query_report`, startDate, endDate, customer);
...
GetDomElement("btnGenerateReport").addEventListener('click', function () {
    ...
    if (bValidForm) {
        var oLaundryRecord = InvokeService(GetURL(), `LogisticNonIndustrialMethods/laundryqueryreport?dateFrom=${dateFrom}&dateTo=${dateTo}&customerID=${GetDomElement("customerId").value}&laundryStatus=${status}`, "GET", "");
        if (oLaundryRecord.code == 200) {
            ...
            if (oData.code == 200) {
                ...
                for (var i = 0; i < oJsonData.length; i++) {
                    SaveTableBody.innerHTML +=
                        "<tr class='border' style='cursor:default'><td class='text-capitalize px-2'>" + oJsonData[i].soDate +
                        "</td><td class='px-1 text-center'>" + oJsonData[i].so_reference +
                        "</td><td class='text-capitalize px-1'>" + capitalize(oJsonData[i].customer_name) +
                        "</td><td class='px-1 text-center'>" + oJsonData[i].weight_in_kg +
                        "</td><td class='px-1 text-center'>" + oJsonData[i].number_of_bags +
                        "</td><td class='px-1'>" + oJsonData[i].received_by_laundry_datetime +
...
function PDFNonIndustrialReport() {
    var status = GetDomElement("selFetchType").value;
    ...
    Download(controllerURL, `${dateToday}-laundry-non-industrial-query-report.pdf`);
}