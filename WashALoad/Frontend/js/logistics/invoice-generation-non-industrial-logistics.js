...
function populateForInvoice() {
    ...
    if (getRadio() == 0) {
        oData = InvokeService(GetURL(), `LogisticNonIndustrialMethods/soforinvoicing?isManualInvoicing=false`, 'GET', '');
    } else {
        oData = InvokeService(GetURL(), `LogisticNonIndustrialMethods/soforinvoicing?isManualInvoicing=true`, 'GET', '');
    }
    if (oData.code == 200) {
...
function populateTableQuery(parsedData) {
    ...
    for (var i = 0; i < parsedData.length; i++) {
        divtbl.innerHTML +=
            "<tr class='border' style='cursor:default'><td class='text-capitalize'><p" + ` style='cursor:pointer;' class='text-primary' data-toggle='modal' data-target='#btnSODetails' onclick = "PopulateNonIndustrialSOReferenceDetails(${parsedData[i].so_reference})"` +
            ">" + parsedData[i].so_reference +
            "</p></td>" +
            "</td><td class='border text-center px-0 '>" + parsedData[i].customer_name + "</td>" +
    ...
    // for (var i = 0; i < 2; i++) {
    //     divtbl.innerHTML +=
    //         "<tr class='border' style='cursor:default'><td class='text-capitalize'><p" + ` style='cursor:pointer;' class='text-primary' data-toggle='modal' data-target='#btnSODetails' onclick = "PopulateNonIndustrialSOReferenceDetails('39')"` +
    //         ">" + 39 +
    //         "</p></td>" +
    //         "<td class='border text-center px-0 text-success'>" + "Test" + "</td>" +
    //         "<td class='border text-center px-0 '>" + "12" + "</td>" +
    //         "</tr>";
    // }
}

function PopulateNonIndustrialSOReferenceDetails(soReference) {
...
GetDomElement('btnMoveSOReference').addEventListener('click', function () {
    ...
    if (arrayTbl.length != 0 && errorInput == false) {
        ...
        GetDomElement("btnConfirmTwo").onclick = () => {
            ...
            var oData = InvokeService(GetURL(), 'LogisticNonIndustrialMethods/generateinvoice', 'POST', strBody);
            ...
            if (oData.code == 200) {
                ...
                if (parsedData.code == 200) {
                    ...
                    // location.href = 'invoice-generation-non-industrial-logistics.html';
                }
