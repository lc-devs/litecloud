GetDomElement('btnSearchReport').addEventListener('click', function () {
    ...
    if (BValidForm) {
        ...
        var oData = InvokeService(GetURL(), `PickupNonIndustrialMethods?dateFrom=${dateFrom}&dateTo=${dateTo}&customerID=${customerId}&pickupBy=${pickUpby}&isAllEntries=${getAllEntries()}&isForPickupOnly=${getAllPickUp()}`, 'GET', '');
...
function populateTableQuery(parsedData) {
    ...
    for (var i = 0; i < parsedData.length; i++) {
        divtbl.innerHTML +=
            ...
            "<td class='border text-center px-0 text-warning text-'>" + `${parsedData[i].booking_reference == '' ? "<label class='text-danger'>N/A</label>" : parsedData[i].booking_reference}` + "</td>" +
...
GetDomElement('pdfile').addEventListener('click', function () {
    ...
    Download(`PDFGenerator/pickupreport?dateFrom=${dateFrom}&dateTo=${dateTo}&customerID=${customerId}&pickupBy=${pickUpby}&isAllEntries=${allEntry}&isForPickupOnly=${isForPickupOnly}&isIndustrial=false`, `pickup-report-nonindustrial-${dateFrom}_${dateTo}`);
