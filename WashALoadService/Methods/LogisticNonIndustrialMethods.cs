...
{
    public class LogisticNonIndustrialMethods
    {
        internal AppDb_WashALoad gDb { get; set; }

        internal CommonFunctions commonFunctions = new CommonFunctions();
        internal LogisticNonIndustrialMethods(AppDb_WashALoad db)
        {
            gDb = db;
        }
        public LogisticNonIndustrialMethods() { }
        ...
        public async Task<ServiceResponse> FindSODetailsAsync(int soReference)
        {
            ...
            try
            {
                ...
                oCommand.CommandText = @"select `so_reference`, 
                                                ...
                                                (COALESCE((SELECT `non_industrial_cost_per_load` FROM `laundry`.`system_parameters`), 0)) as cost_per_load

                                        FROM 
                                        `laundry`.`logistics_non_industrial` i
                ...
                using (oresult)
                {
                    while (await oresult.ReadAsync())
                    {
                        ...
                        jsonObject.cost_per_load = oresult.GetDouble("cost_per_load");
                    }
                }
                ...
                {
                    oCommand.CommandText = @"SELECT ii.`so_reference`, 
	                                                `item_code`, 
	                                        ...
	                                        FROM 
	                                        `laundry`.`logistics_non_industrial_items` ii
	                                        INNER JOIN `laundry`.`non_industrial_laundry_items` li ON li.`id_item` = ii.`item_code`
	                                        INNER JOIN  `laundry`.`logistics_non_industrial_services` nis ON nis.`so_reference` = ii.`so_reference`
                                            WHERE ii.`so_reference` = @so_reference
                    ...
                    oCommand.CommandText = @"SELECT `so_reference`, 
	                                                ...
	                                                FROM 
	                                                `laundry`.`logistics_non_industrial_services`  nis
                    ...
                    using (oresult)
                    {
                        while (await oresult.ReadAsync())
                        {
                            ...
                            jsonService.cost = oresult.GetDouble("cost");
        ...
        public async Task<ServiceResponse> ReceivedQueryReportByDate(string dateFrom, string dateTo, int customerID)
        {
            ...
            try
            {
                ...
                oCommand.CommandText = oCommand.CommandText + " ORDER BY received_from_pickup_datetime";
        ...
        public async Task<ServiceResponse> ReceivedItemByLogisticssAsync(LogisticNonIndustrial logisticNonIndustrial)
        {
            ServiceResponse serviceResponse = new ServiceResponse();
            using var oCommand = gDb.Connection.CreateCommand();
            ...
            try
            {
                ...
                oCommand.CommandText = @"INSERT INTO  `laundry`.`logistics_non_industrial`
                                            (
                                                	`so_reference`, 
	                                                `customer_id`, 
	                                                `booking_reference`, 
	                                    ...
	                                    SELECT 
                                                ...
                                                @received_by_logistics_user,
                                                NOW(),
                                                CEILING(@weight_in_kg/(SELECT `wkg_per_load` FROM `laundry`.`system_parameters`))
                                        
                                             ;";

                oCommand.Parameters.Clear();

                commonFunctions.BindParameter(oCommand, "@so_reference", System.Data.DbType.Int32, logisticNonIndustrial.so_reference);
                commonFunctions.BindParameter(oCommand, "@customer_id", System.Data.DbType.Int32, logisticNonIndustrial.customer_id);
                commonFunctions.BindParameter(oCommand, "@booking_reference", System.Data.DbType.String, logisticNonIndustrial.booking_reference);
                commonFunctions.BindParameter(oCommand, "@picked_up_by", System.Data.DbType.String, logisticNonIndustrial.picked_up_by);
                commonFunctions.BindParameter(oCommand, "@weight_in_kg", System.Data.DbType.Double, logisticNonIndustrial.weight_in_kg);
                commonFunctions.BindParameter(oCommand, "@number_of_bags", System.Data.DbType.Int32, logisticNonIndustrial.number_of_bags);
                commonFunctions.BindParameter(oCommand, "@received_by_logistics_user", System.Data.DbType.String, logisticNonIndustrial.received_by_logistics_user);
                ...
                if (j > 0)
                {
                    ...
                    bool hasData = false;
                    //Insert items -----------------------------------------------------------
                    oCommand.CommandText = @"INSERT INTO  `laundry`.`logistics_non_industrial_items`
                                    (
                                        `so_reference`, 
	                                    `item_code`, 
	                                    `item_count`,
                    ...
                    for (int i = 0; i < logisticNonIndustrial.items.Count; i++)
                    {
                        var oItem = logisticNonIndustrial.items[i];
                        hasData = true;
                        values = values + "(" + logisticNonIndustrial.so_reference.ToString() + ", " + oItem.item_code.ToString() + ", " + oItem.item_count.ToString() + ", '" + oItem.description + "')";

                        if (i < logisticNonIndustrial.items.Count - 1)
                        {
                            values = values + ",";
                        }
                    ...
                    if (hasData)
                    {
                        ...
                        hasData = false;
                        //insert services -----------------------------------------------------------
                        oCommand.CommandText = @"INSERT INTO  `laundry`.`logistics_non_industrial_services`
                                    (
                                        `so_reference`, 
	                                    `service_code`, 
	                                    `cost`
                        ...
                        for (int i = 0; i < logisticNonIndustrial.services.Count; i++)
                        {
                            var oService= logisticNonIndustrial.services[i];
                            hasData = true;
                            values = values + "(" + logisticNonIndustrial.so_reference.ToString() + ", " + oService.service_code.ToString() + ", " + oService.cost +")";

                            if (i < logisticNonIndustrial.services.Count - 1)
                            {
                                values = values + ",";
                            }
                        ...
                        oCommand.CommandText = @"UPDATE  `laundry`.`pickups_non_industrial`
                                            SET
            ...
            try
            {
                ...
                oCommand.CommandText = @"SELECT * FROM(
                                            ...
                                            SELECT `so_reference`, 
	                                                    ...
	                                                    `weight_in_kg`, 
	                                                    `number_of_bags`, 
	                                                    '1900-01-01' AS `received_by_laundry_datetime`, 
	                                                    `completed_by_laundry`,
                                                        `received_from_laundry`,
                                                        `delivered`
        ...
        public async Task<ServiceResponse> ReceivedItemByLaundrysAsync(int soReference, string laundryUser)
        {
            ...
            try
            {
                ...
                oCommand.CommandText = @"UPDATE  `laundry`.`logistics_non_industrial`
                                            SET
            ...
            try
            {
                ...
                if (isManualInvoicing)
                {
                    oCommand.CommandText = @"SELECT i.`so_reference`, 
                                                (
                                                    (COALESCE((SELECT `non_industrial_cost_per_load` FROM `laundry`.`system_parameters`), 0) * `number_of_loads`)
							                        +
							                        (SUM(s.`unit_cost`))
                                                )AS cost, 
                                                c.`customer_name`
                                        FROM 
	                                        `laundry`.`logistics_non_industrial` i
                                        INNER JOIN 
	                                        `laundry`.`logistics_non_industrial_services` d ON i.`so_reference` = d.`so_reference`
	                                INNER JOIN `laundry`.`non_industrial_services` s ON s.`id_service` = d.`service_code` AND `manual_costing` = 1
                                        INNER JOIN 
	                                        `laundry`.`customers` c ON c.`customer_id` = i.`customer_id`
                                        WHERE
	                                        i.`for_invoicing` = 1 AND i.`with_invoice` = 0
                                        GROUP BY i.`so_reference`, c.`customer_name`;";
                }
                else
                {
                    oCommand.CommandText = @"SELECT i.`so_reference`, 
                                        ...
                                        INNER JOIN 
	                                        `laundry`.`logistics_non_industrial_services` d ON i.`so_reference` = d.`so_reference`
	                                INNER JOIN `laundry`.`non_industrial_services` s ON s.`id_service` = d.`service_code` AND `manual_costing` = 0
                                        INNER JOIN 
	                                        `laundry`.`customers` c ON c.`customer_id` = i.`customer_id`
                                        WHERE
	                                        i.`for_invoicing` = 1 AND i.`with_invoice` = 0
        ...
        public async Task<ServiceResponse> GenerateInvoiceAsync(List<ForInvoice> soReferences, string generatedBy)
        {
            ...
            try
            {
                ...
                for (int i = 0; i < soReferences.Count; i++)
                {
                    ...
                    commonFunctions.BindParameter(oCommand, "@invoice_amount", System.Data.DbType.Double, oSOReference.amount);
                    ...
                    if (isSuccess > 0)
                    {
                        oCommand.CommandText = @"INSERT INTO  `laundry`.`non_industrial_invoices`
                                (
                                    `invoice_reference`, 
	                                `so_reference`
                                )
